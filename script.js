...
dailyDateDisplay.readOnly=true;
...
const getDailyTaskItems=(date=selectedDate)=>{
 ...
 return [...calendarEvents, ...saved.filter(task => task.source !== 'event')];
};
...
function selectDate(date){selectedDate=date||selectedDate;dailyDateDisplay.value=new Date(`${selectedDate}T12:00`).toLocaleDateString([],{month:'short',day:'numeric',year:'numeric'});$('dailyDateLabel').textContent=selectedDate===new Date().toISOString().slice(0,10)?'Today':new Date(`${selectedDate}T12:00`).toLocaleDateString([],{weekday:'short',month:'short',day:'numeric'});renderTasks('daily');document.querySelectorAll('.calendar-day').forEach(b=>b.classList.toggle('selected',b.dataset.date===selectedDate));}
...
function addTask(group){const input=$(group==='daily'?'dailyTaskInput':'weeklyTaskInput'),storage=group==='daily'?`dailyTasks:${selectedDate}`:'weeklyTasks',tasks=JSON.parse(get(storage,'[]'));if(!input.value.trim())return;tasks.push({id:Date.now(),label:input.value.trim(),done:false});save(storage,JSON.stringify(tasks));input.value='';renderTasks(group);}
